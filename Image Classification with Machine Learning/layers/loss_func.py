...
class CrossEntropyLoss(BaseLayer):
    ...
    def forward(self, input_x: np.ndarray, target_y: np.ndarray):
        ...
        target_y_ohe = np.zeros((target_y.size, target_y.max() + 1))
        target_y_ohe[np.arange(target_y.size), target_y] = 1
        
        loss = -np.sum(np.sum(np.multiply(target_y_ohe,np.log(input_x)),axis=1))
    ...
    def backward(self):
        ...
        target_y_ohe = np.zeros((y_temp.size, y_temp.max() + 1))
        target_y_ohe[np.arange(y_temp.size), y_temp] = 1

        dx = x_temp - target_y_ohe
